...
setwd(getwd())
source("/Users/taha/Downloads/IBM Data Set/clean_data.R")
...
ui <- fluidPage(
  ...
  dashboardPage(
    dashboardHeader(title = "Telecom Churn Analysis"),
    ...
    dashboardBody(
      ...
      ####geetika
      #      fluidRow(
      #        box(title = "Variable Importance", width = NULL, solidHeader = TRUE, status = "primary",
      #            fluidRow(
      #              tabBox(width = NULL, title = "Variable Importance", side = "right",
      ...
      #        )),
      fluidRow(
        box(title = "Variable Importance", width = NULL, solidHeader = TRUE, status = "primary", color = "#286192",
            fluidRow(
              box(align = "center",
                  title = NULL, 
                  width = 12,
                  status = NULL,
                  box(width = NULL, plotOutput("plot21", height = "250px")))
            ))),      
      
      fluidRow(
        box(title= "Confusion Matrices", width= NULL, solidHeader=TRUE, status="primary",
            fluidRow(
              tabBox(width=NULL, title="Prediction Models",side="right",
                     tabPanel("Deep Learning",
                              fluidRow(
                                box(width=12,title="Deep Learning", plotOutput("plot18"))
                              )
                     ),
                     tabPanel("Random Forest",
                              fluidRow(
                                box(width=13, title="Random Forest", plotOutput("plot19"))
                              )
                     ),
                     tabPanel("GBM",
                              fluidRow(
                                box(width=12, title="GBM", plotOutput("plot20"))
                              )
                     )
              ))
        )
...
server <- function(input, output) {
  ...
  output$avgMonthsNoFilter <- renderInfoBox({
    ...
    #x = format(round(mean(rv$temp_tenure),2))
  ...
  output$plot2 <- renderPlot({
    ...
    plot2 <- ggplot(rv$dataset, aes(x=tenure, fill= Churn))+
      #plot2 <- ggplot(rv$dataset, aes(x=temp_tenure, fill= Churn))+  
  ...
  output$plot18<-renderPlot({
    ...
    p1<-ggplot(data =  rv$df, mapping = aes(x = TClass, y = PClass)) +
      geom_tile(aes(fill = Y), colour = "white") +
      geom_text(size=6,aes(label = sprintf("%1.0f", Y)), vjust = 1) +
      scale_fill_gradient(low = "#FF9933", high = "#00CC66") +
  ...
  output$plot19<-renderPlot({
    ...
    p2<-ggplot(data =  rv$df1, mapping = aes(x = TClass, y = PClass)) +
      geom_tile(aes(fill = Y), colour = "white") +
      geom_text(size=6,aes(label = sprintf("%1.0f", Y)), vjust = 1) +
      scale_fill_gradient(low = "#FF9933", high = "#00CC66") +
      theme_bw() + theme(legend.position = "none", text = element_text(size = 14))
  ...
  output$plot20<-renderPlot({
    ...
    p3<-ggplot(data =  rv$df2, mapping = aes(x = TClass, y = PClass)) +
      geom_tile(aes(fill = Y), colour = "white") +
      geom_text(size=6,aes(label = sprintf("%1.0f", Y)), vjust = 1) +
      scale_fill_gradient(low = "#FF9933", high = "#00CC66") +
      theme_bw() + theme(legend.position = "none", text = element_text(size = 14))
  ...
  output$plot21<-renderPlot({
    ...
    p21 <- ggplot(as.data.frame(rv$var_imp3[1:5,]), aes(x=variable,y=scaled_importance))+
